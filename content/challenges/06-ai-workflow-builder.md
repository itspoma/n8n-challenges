...
During the challenge, give every workflow instruction through your AI chatbot tool. Do not open the n8n editor or manually create, change, connect or run its nodes. Check WordPress.com to confirm that the draft was created.

## Bonus Task
Ask your chatbot to generate one image, add the image as the article header, and publish the finished article on your WordPress.com blog.
...
- [Sign up for OpenRouter](https://openrouter.ai/), [create an API key](https://openrouter.ai/settings/keys), and add it to an [n8n OpenRouter credential](https://docs.n8n.io/integrations/builtin/credentials/openrouter/). Ask your AI chatbot tool to use OpenRouter Chat Model for article generation.
...
- Bonus: publish the article with an AI-generated header image, and provide its public link.
...
- Give your chatbot a clear brief. Open and copy this prompt, then replace the topic and blog placeholders. ||PROMPT|| Check your connected n8n tools first. Build and run a simple workflow entirely through MCP, without asking me to open n8n or configure nodes. My article topic is [TOPIC], my audience is [AUDIENCE], and my WordPress.com blog is [BLOG]. Use the existing OpenRouter connection for text generation and the WordPress.com OAuth2 connection. Generate a useful short article with a title and readable HTML body, and save it as a draft. Choose the necessary nodes yourself. If a required connection or tool is unavailable, explain exactly what is missing. Do not request secrets in chat. Return the execution result and a WordPress draft link.
...
Create workflows through MCP when you want to simplify the whole process and focus on your product, business goals, and desired results. Your AI chatbot tool can choose and configure the nodes and handle the technical details for you.

Use this article workflow to draft news stories from verified sources or create posts for your blog. You could also connect Ahrefs or another keyword research tool to guide topics and create search-focused articles that can help attract relevant traffic.
...
Durante el reto, da todas las instrucciones del workflow a través de tu herramienta de chatbot de IA. No abras el editor de n8n ni crees, modifiques, conectes o ejecutes sus nodos manualmente. Comprueba en WordPress.com que se haya creado el borrador.

## Bonus Task
Pide al chatbot que genere una imagen, coloque la imagen como cabecera del artículo y publique el resultado en tu blog de WordPress.com.
...
- Extra: publica el artículo con una imagen de cabecera generada por IA y proporciona el enlace público.
...
- Extra: pide una imagen de cabecera y publica el artículo. ||PROMPT|| Actualiza el workflow usando solo tus herramientas conectadas. Genera una imagen de cabecera. Sube la imagen a la biblioteca de WordPress.com, insértala al principio del artículo y úsala como imagen destacada si es compatible. Publica el artículo en [BLOG]. Ejecuta el workflow y devuelve el enlace público y el resultado. Comprueba que la página contiene el artículo y la imagen. Si falta una conexión o herramienta, explica el bloqueo sin pedirme editar nodos manualmente.
- Abre el enlace final y revisa el texto, la imagen y el estado publicado. Si falta algo, pide al chatbot que lo corrija y vuelva a ejecutar el workflow.
...
Crea workflows mediante MCP cuando quieras simplificar todo el proceso y centrarte en tu producto, tus objetivos de negocio y los resultados que buscas. Tu herramienta de chatbot de IA puede elegir y configurar los nodos y encargarse de los detalles técnicos.

Usa este workflow para redactar noticias a partir de fuentes verificadas o crear publicaciones para tu blog. También puedes conectar Ahrefs u otra herramienta de investigación de palabras clave para elegir temas y crear artículos orientados a búsquedas que puedan atraer tráfico relevante.
...
Під час завдання давай всі інструкції для воркфлоу через свій інструмент ШІ-чатбота. Не відкривай редактор n8n і не створюй, не змінюй, не з’єднуй та не запускай його ноди вручну. Перевір у WordPress.com, що чернетку створено.

## Bonus Task
Попроси чатбот згенерувати одне зображення, додати його на початок статті й опублікувати результат у блозі WordPress.com.
...
- [Зареєструйся в OpenRouter](https://openrouter.ai/), [створи API-ключ](https://openrouter.ai/settings/keys) і додай його до [облікових даних OpenRouter у n8n](https://docs.n8n.io/integrations/builtin/credentials/openrouter/). Попроси свій інструмент ШІ-чатбота використати OpenRouter Chat Model для генерації статті.
...
- Бонус: опублікуй статтю зі згенерованим зображенням на початку та надай публічне посилання.
...
- Дай чітке завдання. Відкрий і скопіюй промпт та заміни поля. ||PROMPT|| Перевір свої інструменти n8n. Створи й запусти воркфлоу через MCP, не просячи мене відкривати n8n чи налаштовувати ноди. Тема: [ТЕМА]. Аудиторія: [АУДИТОРІЯ]. Блог WordPress.com: [БЛОГ]. Використай готове підключення OpenRouter для генерації тексту та підключення WordPress.com OAuth2. Згенеруй коротку статтю із заголовком і читабельним HTML та збережи чернетку. Обери ноди самостійно. Якщо бракує підключення чи інструмента, поясни якого саме. Не проси секрети в чаті. Поверни результат виконання та посилання на чернетку.
...
Створюй воркфлоу через MCP, коли хочеш спростити весь процес і зосередитися на продукті, бізнес-цілях та бажаних результатах. Твій інструмент ШІ-чатбота може сам обирати й налаштовувати ноди та розбиратися з технічними деталями.

Використовуй цей воркфлоу для підготовки новин із перевірених джерел або дописів для свого блогу. Також можна підключити Ahrefs чи інший інструмент дослідження ключових слів, щоб обирати теми й створювати статті під пошукові запити, які можуть залучати відповідну аудиторію.
